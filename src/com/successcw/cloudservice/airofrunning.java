...
//						PrimaryPollutant = "PM2_5";
			...
			for (int i = 0; i < city.length; i++) {
				if(i == 0 || i == 19) {
			        ...
			        try{
						DataSetList result = emcClient.getAQIByCityName(city[i]);

						JSONObject json = new JSONObject(responseStrBuilder.toString());
						province = json.getJSONArray("provinces");
						for(temp_i = 0; temp_i < province.length(); temp_i++) {
							cityJson = province.getJSONObject(temp_i).getJSONArray("cities");
							for(temp_j = 0; temp_j < cityJson.length(); temp_j++){
								if(cityJson.getJSONObject(temp_j).getString("name").equals(city[i])) {
									flag = 1;
									break;
								}
							}
						...
						station = cityJson.getJSONObject(temp_j).getJSONArray("stations");
						System.out.print("cityJson=" + cityJson.getJSONObject(temp_j).getString("name"));
						...
						for(temp_i = 0; temp_i < station.length(); temp_i++) {
							for(temp_j = 1; temp_j < result.nameList.size(); temp_j=temp_j+12) {
								if(station.getJSONObject(temp_i).getString("name").equals(result.valueList.get(temp_j))) {
									Integer AQI = TryParseInt(result.valueList.get(temp_j+1));
									if (AQI == null)
										AQI = -9999;
									else {
										averageAQI += AQI;
									...
									NO2 = -9999;
									try{
										PM2_5 = Float.parseFloat(result.valueList.get(temp_j+2));
									} catch(NumberFormatException e){
										PM2_5 = -9999;
									}
									try{
										PM10 = Float.parseFloat(result.valueList.get(temp_j+3));
									} catch(NumberFormatException e){
										PM10 = -9999;
									}
									try{
										SO2 = Float.parseFloat(result.valueList.get(temp_j+4));
									} catch(NumberFormatException e){
										SO2 = -9999;
									}
									try{
										NO2 = Float.parseFloat(result.valueList.get(temp_j+5));
									} catch(NumberFormatException e){
										NO2 = -9999;
									}
									try{
										CO = Float.parseFloat(result.valueList.get(temp_j+9));
									} catch(NumberFormatException e){
										CO = -9999;
									}
									try{
										O3_1H = Float.parseFloat(result.valueList.get(temp_j+6));
									} catch(NumberFormatException e){
										O3_1H = -9999;
									}
									try{
										O3_8H = Float.parseFloat(result.valueList.get(temp_j+7));
									} catch(NumberFormatException e){
										O3_8H = -9999;
									}
									Integer temp = i + 1;
									SQL = "UPDATE airQuality SET averageAQI=" + averageAQI/count + "," +
											"AQI=" + AQI +
											",time='" + result.valueList.get(temp_j+8).toString().split(" ")[1].toString().substring(0,8) + "'" + "," +
											"station='" + result.valueList.get(temp_j) + "'" + "," +
											"quality='" + "-9999"  + "'" + "," +
											"PrimaryPollutant='" + PrimaryPollutant + "'" + "," +
											"PM2_5=" + PM2_5 + "," +
											"PM10=" + PM10 + "," +
											...
											"O3_1H=" + O3_1H + "," +
											"O3_8H=" + O3_8H +
											" WHERE cityIndex=" + temp + " AND stationIndex=" + temp_i;
